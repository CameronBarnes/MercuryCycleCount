...
public class IngestForm {
	...
	public IngestForm(Session session) {
		...
		mNextButton.addActionListener(e -> {
			mSession.count();
		});
		...
		mOpenButton.addActionListener(e -> mSession.addIngest(FileSystemUtils.getDirectoryWithFileChooser()));
		...
		mAddEmptyBin.addActionListener(e -> {
			...
			while (!validBin) {
				...
				String tmp = JOptionPane.showInputDialog("Enter the name of the empty bin.");
				if (tmp == null)
				...
				if (input.matches(CountForm.BIN_NO_PATTERN)) {
					...
					if (mSession.getBins().stream().anyMatch(bin -> bin.getBinNum().equalsIgnoreCase(input))) {
						...
						JOptionPane.showMessageDialog(mPanel, "A bin with that bin number has already been loaded in.", "Bin Already Present", JOptionPane.WARNING_MESSAGE);
						continue;
					}
					validBin = true;
					String wareHouse = "";
	...
	private void $$$setupUI$$$() {
		...
		Font mOpenButtonFont = this.$$$getFont$$$(null, -1, 16, mOpenButton.getFont());
		if (mOpenButtonFont != null) mOpenButton.setFont(mOpenButtonFont);
		mOpenButton.setText("Select Folder");
		mPanel.add(mOpenButton, new GridConstraints(0, 3, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_HORIZONTAL, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
		...
		label1.setText("Load Bin Excel Files");
		...
		label2.setText("Place the bin status spreadsheets into the Import folder, click the button to select a folder you'd like to import from, or drag the spreadsheet into this window.");
		...
		mClearButton.setText("Remove All");
		...
		mAddEmptyBin.setText("Add Empty Bin");
